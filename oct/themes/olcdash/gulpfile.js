...
gulp.task('browser-sync', function() {
    browserSync({
        ...
        proxy: {
            target: "olcdash/" // Enter your dev environment proxy
        }
    });
    gulp.watch('assets/css/stylus/*.styl', ['stylus']).on('change', reload); // Watches all Stylus files for changes
    gulp.watch('assets/styles/styler.scss').on('change', reload); //watch foundation scss
    gulp.watch('assets/javascript/main.js').on('change', reload); //watch foundation main.js
});
